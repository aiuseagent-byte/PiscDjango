...
# definição para saida estruturada da llm (pydentic)
...
class BaseAgent:
    ...
    # ao iniciar um método com _ indica por convenção que é um método privado da classe
    @abstractmethod
    def _prompt(self):...
...
# Agente para avaliar humar do paciente
...
class RAGContext:
    ...
    def retrieval(self, id_pergunta, id_paciente, k=5):
        ...
        # TODO refatorar para Django BookCreate para otimização de performace ---
        # Salva no banco a referencia da transcrição referente a pergunta
        for doc in docs:
        ...
        # contexto = ''
        # for doc in docs:
        #     contexto += f"Material: {doc.page_content} \n\n"
        ...
        # Generator Yield -> Permite consumir os dados conforme a necessidade  (Receber os dados aos pouco e não apenas quando terminar)
        # (Ou seja conforme os streams vão sendo gerado, o modelo vai devolvendo os tokens vamos mostrando e dando a resposta)
        # Gererator do python
        for chunk in self.chat.stream(messages):
            token = chunk.content
            # quando modelo devolver NONE sabemos que finalizou
            if token:
                yield token 
